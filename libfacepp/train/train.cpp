...
void train::Identify()
{
	pplx::task<void> requestTask = concurrency::streams::fstream::open_ostream(U("trains.json")).then([=](concurrency::streams::ostream outFile)
	{
		http_client client(API_SERVER);
		query.append_path(U("/train/identify"));
...
#endif
		wofstream res_json("trains.json");
